import {Button} from "@/components/ui/button.tsx";
import {Navbar} from "@/components/ui/navbar.tsx";
import {useNavigate, useParams} from "react-router-dom";
import {z} from "zod";
import {useForm} from "react-hook-form";
import {zodResolver} from "@hookform/resolvers/zod";
import {useState} from "react";
import {Alert, AlertDescription, AlertTitle} from "@/components/ui/alert.tsx";
import {AlertCircle, Terminal} from "lucide-react";
import {Form, FormControl, FormField, FormItem, FormLabel, FormMessage} from "@/components/ui/form";
import {Input} from "@/components/ui/input.tsx";
import {axiosInstance} from "@/lib/axios.ts";
import {useTranslation} from "react-i18next";

export const VerifyForgotPasswordPage = () => {
    const {t} = useTranslation();

    const [error, setError] = useState("");
    const [success, setSuccess] = useState("");
    const {forgotPasswordToken} = useParams<{ forgotPasswordToken: string }>();
    const navigate = useNavigate();

    const verifyForgotPasswordFormValidationSchema = z.object({
        newPassword: z.string().min(6, t("Guest.VerifyForgotPassword.error-new-password-len")),
        newPasswordRepeated: z.string().min(6, t("Guest.VerifyForgotPassword.error-new-password-repeated-len")),
    });

    const verifyForgotPasswordForm = useForm<z.infer<typeof verifyForgotPasswordFormValidationSchema>>({
        resolver: zodResolver(verifyForgotPasswordFormValidationSchema),
        defaultValues: {
            newPassword: "",
            newPasswordRepeated: ""
        }
    });

    const handleSubmit = async () => {
        setError("");
        setSuccess("");

        const values = verifyForgotPasswordForm.getValues();

        if (values.newPassword!==values.newPasswordRepeated) {
            setError(t("Guest.VerifyForgotPassword.error-passwords-must-match"));
            return;
        }

        await axiosInstance
                .post(`auth/${forgotPasswordToken}/verify-forgot-password`, verifyForgotPasswordForm.getValues())
                .then(() => {
                    setSuccess(t("Guest.VerifyForgotPassword.success"));
                    setTimeout(() => {
                        navigate("/login");
                    }, 1000);
                })
                .catch(e => {
                    let key = "Guest.VerifyForgotPassword.error-fallback";

                    const exceptionMessage = e.response.data.ExceptionMessage.toLowerCase();

                    if (exceptionMessage.includes("user")) {
                        key = "Guest.VerifyForgotPassword.error-user-not-found";
                    } else if (exceptionMessage.includes("old")) {
                        key = "Guest.VerifyForgotPassword.error-password-must-differ";
                    }

                    setError(t(key));
                });
    }

    return (
            <div className="flex flex-col min-h-screen">
                <Navbar/>
                <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-10">

                    <div className="text-center my-10">
                        <h1 className="text-3xl font-bold">
                            {t("Guest.VerifyForgotPassword.form")}
                        </h1>
                        <p className="my-10 text-muted-foreground text-xl whitespace-pre-line">
                            {t("Guest.VerifyForgotPassword.description")}
                        </p>
                    </div>

                    <Form {...verifyForgotPasswordForm}>
                        <form onSubmit={verifyForgotPasswordForm.handleSubmit(handleSubmit)}
                              className="w-full max-w-2xl mx-auto space-y-8 flex flex-col"
                        >
                            <FormField
                                    control={verifyForgotPasswordForm.control}
                                    name="newPassword"
                                    render={({field}) => (
                                            <FormItem>
                                                <FormLabel>{t("Guest.VerifyForgotPassword.new-password")}</FormLabel>
                                                <FormControl>
                                                    <Input
                                                            placeholder="**********"
                                                            type="password"
                                                            {...field} />
                                                </FormControl>
                                                <FormMessage/>
                                            </FormItem>
                                    )}
                            >
                            </FormField>

                            <FormField
                                    control={verifyForgotPasswordForm.control}
                                    name="newPasswordRepeated"
                                    render={({field}) => (
                                            <FormItem>
                                                <FormLabel>{t("Guest.VerifyForgotPassword.new-password-repeated")}</FormLabel>
                                                <FormControl>
                                                    <Input
                                                            placeholder="**********"
                                                            type="password"
                                                            {...field} />
                                                </FormControl>
                                                <FormMessage/>
                                            </FormItem>
                                    )}
                            >
                            </FormField>

                            <Button type="submit">
                                {t("Common.submit")}
                            </Button>

                            {error && (
                                    <Alert variant="destructive">
                                        <AlertCircle className="h-4 w-4"/>
                                        <AlertTitle>{t("Common.error")}</AlertTitle>
                                        <AlertDescription>
                                            {error}
                                        </AlertDescription>
                                    </Alert>
                            )}
                            {success && (
                                    <Alert>
                                        <Terminal className="h-4 w-4"/>
                                        <AlertTitle>{t("Common.success")}</AlertTitle>
                                        <AlertDescription>
                                            {success}
                                        </AlertDescription>
                                    </Alert>
                            )}

                        </form>
                    </Form>
                </div>
            </div>
    );
}